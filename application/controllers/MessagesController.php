...
/* print_r($ids);die; */
		...
		if($status =='trash'){
			 $updateWhere[] = sprintf('author_id = %d', Application_Service_Authorization::getInstance()->getUserId());
        if (!empty($ids)) {
    ...
    public function deleteCalendarNoteAction()
    {
         try {
            $req = $this->getRequest();
            $noteId = $req->getParam('id');
            $note = Application_Service_Utilities::getModel('Notes');
            $note->remove($noteId);
            $this->flashMessage('success', 'Note Deleted !');
            $this->redirect('home');
         }catch (Exception $e) {
            $this->_helper->getHelper('flashMessenger')->addMessage($this->showMessage('error in deleting note', 'danger'));
        }
    }
...
//push data to google api
    public function push(Array $data)
    {
      ...
      //data to add to google
        ...
        $startTime = date('h:i:s', strtotime($data['date_start']));
        $endTime = date('h:i:s', strtotime($data['date_end']));

        $sdate = $startDate.'T'.$startTime;
        $edate = $endDate.'T'.$endTime;

        $dataToModel['sdate'] = $sdate;
        $dataToModel['edate'] = $edate;
        $dataToModel['summary'] = $data['title'];
        $dataToModel['description'] = $data['content'];
        $dataToModel['user_id'] = Application_Service_Authorization::getInstance()->getUserId();
        $this->googleModel->save($dataToModel);

    //send notification .. NO attendees so no notification.
