...
class OsobyController extends Muzyka_Admin
{
    ...
    public function init()
    {
        ...
        $this->registryEntries = Application_Service_Utilities::getModel('RegistryEntries');
        $this->personsService = Application_Service_Osoby::getInstance();
    ...
    public function addminiAction()
    {
        ...
        if (!empty($this->_getParam('template')) && ($this->_getParam('template') == 1)){
            /*if (!empty($this->_getParam('tid'))){

            }*/
            $t_data = array();
            $workers = Application_Service_DocumentsPrinter::getInstance()->getActiveWorkersList();
    ...
    public function addminiForusergroupsAction()
    {
        ...
        $this->view->ajaxModal = 1;
        $t_data =$groupsModel->fetchAll()->toArray();
    ...
    public function updateAction()
    {
        ...
        if ($id) {
            ...
            $rightsPermission = json_decode($row->rightsPermissions);
            $rightsPermissionsArray = json_decode($row->rightsPermissions, true);

            if ($rightsArray === null) {
                $rightsArray = [];
            }

            if ($rightsPermissionsArray === null) {
                $rightsPermissionsArray = [];
            }
            ...
            $assignedWorkerIds = $this->osoby->getAssignedWorkerIds([$data->assigned_worker_id]);
        } else {
            $this->setDetailedSection('Dodaj pracownika');
            $data->generate_documents = 1;
            $this->view->pageName = 'create';
            $data['assigned_worker_id'] = -1;
        }
        // comagom code start
        $modelAdminLink = Application_Service_Utilities::getModel('AdminLink');
        $expertUser = Application_Service_Authorization::getInstance()->getUserId();    
        $usersCountsFromDB = $modelAdminLink->getUsersCountByAdmin($expertUser);
        $licenseSubscriptionService = Application_Service_LicenseSubscriptions::getInstance();
        $license = $licenseSubscriptionService->getActive($expertUser, Application_Model_LicenseSubscription::STATUS_ACTIVATED);
       $usersCountsInfo = array(
            'expert' => '0/'.$license->{'expert_count'},
            'pro' => '0/'.$license->{'pro_count'},
            'mini' => '0/'.$license->{'mini_count'}
        );
        foreach($usersCountsFromDB as $key => $value) {
            if($license) {
                $usersCountsInfo[$value['type']] = $value['count']. '/'. $license->{$value['type'].'_count'};
            }
        }
        //comagom code end
        /** @var Application_Model_Role $role */
        $role = Application_Service_Utilities::getModel('Role');
        $programy = Application_Service_Utilities::getModel('Applications');
        /** @var Application_Model_Zbiory $zbioryModel */
        $zbioryModel = Application_Service_Utilities::getModel('Zbiory');
        /** @var Application_Model_Groups $groupsModel */
        $groupsModel = Application_Service_Utilities::getModel('Groups');
        /** @var Application_Model_AdminLink $adminLink */
        $adminLink = Application_Service_Utilities::getModel('AdminLink');
        /** @var Application_Model_OsobyGroups $osobyGroupsModel */
        $osobyGroupsModel = Application_Service_Utilities::getModel('OsobyGroups');
        ...
        if ($user) {
            ...
            $typeRights = json_decode($this->rightsModel->getBytype($osobyType['type']), true);

            $typeRightsPermissions = json_decode($this->rightsPermissionsModel->getBytype($osobyType['type']), true);
        }

        //for permissions of registry
        ...
        // var_dump(json_encode($typeRightsPermissions, JSON_PRETTY_PRINT));
        // exit;

        // comagom code start
        $this->view->usersTypesCounts = $usersCountsInfo;
        $this->view->license_selected_id = $license->license_id;
        // comagom code end

        $this->view->registries = $res;
        $this->view->typeRightsPermissions = $typeRightsPermissions;
        $this->view->rightsPermissions = $this->userRights($rightsPermissions);
        $this->view->rightsPermissionsArray = $rightsPermissionsArray;

        // comagom code start 2019.3.19
        $workers = Application_Service_DocumentsPrinter::getInstance()->getActiveWorkersList([]);
        
        //echo '<pre>';print_r($data['assigned_worker_id']);echo '</pre>';exit;
        //$assignedWorkerIds);
        // get workers which already was assigned by users.
        $assignedWorkersList = $this->osoby->getAssignedWorkerIds();
        $assignedWorkers = array();
        if ($assignedWorkersList) {
            foreach ($assignedWorkersList as $value) {
                $assignedWorkers[] = $value['assigned_worker_id'];
            }
        }
        // if (!in_array($data['assigned_worker_id'], $workers)) {
        //     $data['assigned_worker_id'] = "";    
        // }
        //echo '<pre>';print_r($assignedWorkersList);echo '</pre>';exit;
        //unsert assigned workers from workers list.
        $is_present = false;
        if ($assignedWorkersList) {
            foreach ($workers as $k => $worker) {
                if ($k == $data['assigned_worker_id']) {
                     $is_present = true;
                }
                if (in_array($k, $assignedWorkersList) && $k != $data['assigned_worker_id']) {
                    unset($workers[$k]); 
                }
            }
        }
        if (!$is_present) {
            ...
            $documentsTemplatesOsoby = Application_Service_Utilities::getModel('Documenttemplatesosoby')->getAllDocumentTemplatesIds($data['assigned_worker_id']);
            foreach ($documentsTemplatesOsoby as $key => $dto) {
                $is_active = Application_Service_Utilities::getModel('Documenttemplates')->isActive($dto);
                if ($is_active == 1) {
                    $task = $this->tasksModel->getTaskByObjectId($dto);
                    if (isset($task[0])) {
                        $checkExisting = $this->StoragetasksModel->checkExisting($dto,$task[0]['id'],$id);
                        if (empty($checkExisting)) {
                            $currentDate = date('Y-m-d');
                            $tasksService->createStorageTaskSimple($task[0],$dto,$id,$currentDate);
                        }
                    }
                }
               }    
        ...
        $this->view->typeRights = $typeRights;
        $this->view->rightsConfigExtended = Application_Service_Authorization::getInstance()->getModuleSettingsSorted($rightsArray);
        $this->view->taskId = $taskId;

        $this->view->user = $user;
        $this->view->adminLink = $osobyType['type'];
        $this->view->data = $data;
        // comagom code start
        $this->view->isAdmin = Application_Service_Authorization::getInstance()->isAdmin();
        // comagom code end
        $this->view->licenseTab = Application_Service_Authorization::getInstance()->isSuperAdmin();
        $this->view->typesOfUser = $this->personsService->getUserTypes();
        $this->view->workers = $workers;
        $this->view->selectedOsobyId = $id;
        ...
        $licenseSubscriptionService = Application_Service_LicenseSubscriptions::getInstance();
        $licenses = $licenseService->getList();
        $licenseTrialDates = [];
        foreach ($licenses as $license) {
            $licenseTrialDates[$license->id] = $licenseService->getTrialEndDate($license->id);
        }
        $this->view->licenses = $licenses;
        $this->view->licenseSubscriptions = $licenseSubscriptionService->getList(['osoby_id'=>$id]);
        $this->view->licenseTrialDates = $licenseTrialDates;
        $this->view->isCompany = $this->usersModel->getCompanyAdmin();
    }
    ...
    public function saveAction()
    {
        ...
        $redirectDeployUrl='/activation';
        $pageName = $req->getParam('page_name', 'update');
        $proposalRole = $req->getParam('proposal_role', '');
        $accType = $req->getParam('empType');
        $stopRedirect=$this->_request->getPost('stopredirect','');
        if($accType==''){
           $accType=$aPost['empType'];
        }
        $id = $req->getParam('id', 0);
        $modelAdminLink = Application_Service_Utilities::getModel('AdminLink');
        
        $logic = new Logic_Users();
        ...
        try {
            //$this->usersModel->getCompanyAdmin();
            $expertUser = Application_Service_Authorization::getInstance()->getUserId();    
            
            ////reference today for getting osoby user by $accType
            $login_do_systemu = '';
            
            if($id) {
                $row = $this->osoby->getOne($id);
                $login_do_systemu = $row->login_do_systemu;
               
            }
            
            $subOsoby = $modelAdminLink->getUsersByAdmin($expertUser, $accType, $login_do_systemu);
            
            if (!empty($subOsoby)) {
                $activeCount = Application_Service_Utilities::getModel('Osoby')->countActive($subOsoby);
                $licenseSubscriptionService = Application_Service_LicenseSubscriptions::getInstance();
                //$license = $licenseSubscriptionService->getActive(Application_Service_Authorization::getInstance()->getUserId(), Application_Model_LicenseSubscription::STATUS_ACTIVATED);
                $license = $licenseSubscriptionService->getActive(null, Application_Model_LicenseSubscription::STATUS_ACTIVATED);
                $allowedCount = 0;
                if($license)
                {
                    $allowedCount = $license->{$accType.'_count'};
                }
                if ($activeCount >= $allowedCount && ($req->getParam('status') != 1 || $pageName == 'create') && !Application_Service_Authorization::getInstance()->isSuperAdmin()) {
                $this->_helper->getHelper('flashMessenger')->addMessage($this->showMessage('Limit użytkownika przekracza. Użytkownik został dezaktywowany. Uaktualnij swoje konto lub zmień typ użytkownika', 'warning'));
                $this->_redirect("osoby/update");
                } 
            }

            if($id) {
                if(isset($aPost['assigned_worker_id'])){
                    $assignedWorkerIds = $this->osoby->getAssignedWorkerIds([$data->assigned_worker_id]);
                    
                    if($assignedWorkerIds){
                        $aPost['assigned_worker_id'] = $assignedWorkerIds;
                    } else {
                        $aPost['assigned_worker_id'] = -1;
                    }
                } else {
                    $aPost['assigned_worker_id'] = -1;
                }

                $logic->assignTasks($id, $aPost);
            }
            ...
            $modelNotifyControl = Application_Service_Utilities::getModel('NotificationsControl');
            $modelNotifyControl->create($osoba->id);

            $modelAdminLink = Application_Service_Utilities::getModel('AdminLink');
            $modelAdminLink->create(Application_Service_Authorization::getInstance()->getUserId(), $osoba->login_do_systemu, $accType);
            ...
            if ($req->getParam('submit_type') !== 'task_complete') {
                ...
                $documentsService->updatePendingDocuments(array($osoba->id));

                if ($osoba->assigned_worker_id != '') {
                    $this->tasksModel = Application_Service_Utilities::getModel('Tasks');
                    $this->StoragetasksModel = Application_Service_Utilities::getModel('StorageTasks');
                    $tasksService = Application_Service_Tasks::getInstance();
                    $documentsTemplatesOsoby = Application_Service_Utilities::getModel('Documenttemplatesosoby')->getAllDocumentTemplatesIds($osoba->assigned_worker_id);
                    
                    foreach ($documentsTemplatesOsoby as $key => $dto) {
                        $is_active = Application_Service_Utilities::getModel('Documenttemplates')->isActive($dto);
                        $hasDocument = $logicDocuments->hasWorkerActiveDocumentByTemplateId($osoba->assigned_worker_id, $dto);
                        
                        if ($is_active == 1 && $hasDocument) {
                            $task = $this->tasksModel->getTaskByObjectId($dto);
                            
                            if (isset($task[0])) {
                                $checkExisting = $this->StoragetasksModel->checkExisting($dto, $task[0]['id'], $osoba->id);
                                if (empty($checkExisting)) {
                                    $currentDate = date('Y-m-d');
                                    $tasksService->createStorageTaskSimple($task[0], $dto, $osoba->id, $currentDate);
                                }
                            }
                        }
                    }
        ...
        if ($req->getParam('submit_type') === 'task_complete') {
            ...
            if ($stopRedirect != '') {
                $this->_redirect($redirectDeployUrl . '?email=' . $aPost['email']);
            } else {
    ...
    public function kopiujosobeAction()
    {
        ...
        if ($role_chbx) {
            $this->personsService->clearRoles($osobaId);
            $this->personsService->saveRoles($roles, $osobaId);
        }
        ...
        if ($klucze_chbx) {
            ...
            if (array_diff($klucze, $kluczeNowego)) {
                ...
                $this->personsService->clearKlucze($osobaId);
                $this->personsService->saveKlucze($kluczeSum, $osobaId);
   ...
   /* public function podpisyAction()
    ...
    public function upowaznieniaTransferGoAction()
    {
        ...
        try {
            ...
            foreach ($users as $userId) {
                ...
                $this->personsService->saveKlucze($pomieszczeniaTarget, $userId);
            }
    ...
    public function resetPasswordSaveAction()
    {
        ...
        $getUser = $usersModel->getUserByEmail($osoba->email);
        $admin = $getUser->isAdmin;
        $usersModel->savePassword($osoba, $password, $admin, true);
        //$usersModel->savePassword($osoba, $password, 0, true);
    ...
    public function resetPasswordEmailSaveAction()
    {
        $id = $this->_getParam('id');
        $userModel = Application_Service_Utilities::getModel('Users');
        ...
        if ($user) {
            ...
            $dataRecov = array ('recovery_key' => $recoveryKey);
            $userModel->editRec($user->id, $dataRecov);
            ...
            try {
                ...
                $this->sendSmtpMail($bodyText, 'Reset Password', $osoba->email, $emailPrzedstawiciela);
    ...
    public function getPendingDocumentId($osobaId)
    {
        $documentTemplateOsoby = $this->documenttemplatesosoby->getOne($osobaId);
        $documentTemplateId = $documentTemplateOsoby->documenttemplate_id;

        $pendingDocumentId = $this->documentsPending->getPendingDocumentId($osobaId, $documentTemplateId);
     ...
     * @throws Exception
     */
    private function processSaveOsoba(Zend_Controller_Request_Http $req)
    {
        
        try {
            $osoba = $this->personsService->save($req);
            
            $isAdmin = Application_Service_Authorization::getInstance()->isAdmin();
            $isSuperAdmin = Application_Service_Authorization::getInstance()->isSuperAdmin();
            if($isAdmin == 1 && $isSuperAdmin == 1) {
                if(!empty($req->getPost())){
                    $licenseId = $req->getPost('license_id');
                    $license_status = $req->getPost('license_status');
                    $endDate = $req->getPost('license_end_date');
                }else{
                    $licenseId = $req->getParam('license_id');
                    $license_status = $req->getParam('license_status');
                    $endDate = $req->getParam('license_end_date');
                }
                
            } elseif( $isAdmin == 1) {
                if(!empty($req->getPost())){
                    $licenseId = $req->getPost('license_selected_id','');
                }else{
                    $licenseId = $req->getParam('license_selected_id','');
                }
                $license_status = 1;
                $expertUser = Application_Service_Authorization::getInstance()->getUserId();
                $licenseSubscriptionModel = Application_Service_Utilities::getModel('LicenseSubscription');
                $endDate = $licenseSubscriptionModel->getEndDateByOsobyId($expertUser);
            }
            if ($licenseId) {
                $licenseSubscriptionService = Application_Service_LicenseSubscriptions::getInstance();
                ...
                foreach ($licenseSubscriptionService->getList($osoba->id, Application_Model_LicenseSubscription::STATUS_ACTIVATED) as $subscription) {
                ...
                //$subscription = $licenseSubscriptionService->create($licenseId, $osoba->id, $endDate);
                $subscription = $licenseSubscriptionService->create($licenseId, $osoba->id, $endDate, Application_Model_LicenseSubscription::STATUS_INACTIVE, $arrTypeCounts);
               // get details  if user is admin or not 
                $usersModel = Application_Service_Utilities::getModel('Users');
                $getUser = $usersModel->getUserByLogin($osoba->login_do_systemu);
                $admin = $getUser->isAdmin;
                if($admin){
                    if($isAdmin == 1 && $isSuperAdmin == 1) {
                    // get list of all user 
                          $allUsersOfAdmin=$this->osoby->getList(['o.type = 1', 'o.usunieta = 0']);
                            if(!empty($allUsersOfAdmin)){
                                foreach($allUsersOfAdmin as $allLicense){
                                foreach ($licenseSubscriptionService->getList($allLicense->id, Application_Model_LicenseSubscription::STATUS_ACTIVATED) as $subscription) {
                              ...
                              $subscription = $licenseSubscriptionService->create($licenseId, $allLicense->id, $endDate, Application_Model_LicenseSubscription::STATUS_INACTIVE, $arrTypeCounts);
                              if ($license_status) {
                                        $licenseSubscriptionService->approve($subscription);
                                }
                            } 
                        }
                    }
            ...
            return $osoba;
        } catch (Application_Service_Exception_OsobyException $e) {
            $this->flashMessage('error', $e->getMessage());
            $this->_redirect($_SERVER ['HTTP_REFERER']);
        } catch (Application_SubscriptionOverLimitException $e) {
            $this->_redirect('subscription/limit');
        } catch (Exception $e) {
