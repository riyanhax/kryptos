...
class ApiController extends Muzyka_Action {
    ...
    public function scheduleNotificationsAction() {
        $notificationsServerService = Application_Service_NotificationsServer::getInstance();
        $notifications = $this->_getParam('notifications');
        $results = [];
    ...
    public function freeTrialAction()
    {
        ...
        if (!$email || !$phone) {
            $this->sendReturnStatus('Not found', 404);
            return;
        }
        $freeTrialService = Application_Service_FreeTrials::getInstance();
        if (!$freeTrialService->checkSign($sign, [$email, $phone])) {
            $this->sendReturnStatus('Sign is not valid', 403);
            return;
        }
        try {
            $result = $freeTrialService->create($email, $phone);
            $this->sendReturnStatus([
                'message' => 'Free trial successfully created',
                'confirmation_code' => $result->getConfirmationCode(),
            ]);
        } catch (Application_Service_Exception_DuplicateException $e) {
            $this->sendReturnStatus($e->getMessage(), 422);
        } catch (Exception $e) {
            $this->sendReturnStatus($e->getMessage(), 500);
        }
    }

    /* Vipin code starts */
    public function checkTrialEmailAction()
    {
        $this->sendAccessControlHeaders();
        $request = $this->getRequest();
        $email = $request->getPost('email');
        ...
        $query = $db->select()->from('free_trials')->where('email =?', $email)->where('status =?', 0);
        ...
        if($email == $row['email']){
            ...
            $this->sendReturnStatus([
                'message' => 'Email jest inny niż podany przy rejestracji',
                'api_status' => 0
            ]);
        }
     ...
     * @throws Exception
     */
    public function freeTrialConfirmAction()
    {
        $this->sendAccessControlHeaders();
        /** @var Zend_Controller_Request_Http $request */
        $request = $this->getRequest();
        ...
        $email = $request->getPost('email');
        $confirmationCode = $request->getPost('code');
        $sign = $request->getPost('sign');
        ...
        $freeTrialService = Application_Service_FreeTrials::getInstance();
        if (!$freeTrialService->checkSign($sign, [$email, $confirmationCode])) {
            $this->sendReturnStatus('Sign is not valid', 403);
            return;
        }
        try {
            ...
            $this->sendReturnStatus([
                'message' => 'Free trial successfully confirmed',
                'data' => $json_encode,
            ...
            $db->commit();
            /* Vipin code end */
        } catch (Application_Service_Exception_NotFoundException $e) {
            $this->sendReturnStatus($e->getMessage(), 404);
        } catch (Application_Service_Exception_DuplicateException $e) {
            $this->sendReturnStatus($e->getMessage(), 422);
        } catch (Application_Service_Subsription_Exception_BadRequestException $e) {
            $this->sendReturnStatus($e->getMessage(), 400);
        } catch (Exception $e) {
            $this->sendReturnStatus($e->getMessage(), 500);
        }
    }

    // return user details  for creating superadmin
      public function createAdminAction()
    {
        ...
        $db = Zend_Db_Table::getDefaultAdapter();
        $query = $db->select()->from('free_trials')->where('email =?', $email)->where('status =?', 1);
        $row = $db->fetchRow($query);
        if($email == $row['email']){
            $this->sendReturnStatus([
                'message' => 'Email is valid',
    ...
    public function subscriptionCallbackAction()
    {
        /** @var Zend_Controller_Request_Http $request */
        $request = $this->getRequest();
        Application_Service_LicenseSubscriptions::getInstance()
            ->subscriptionChangeCallback($request);
        $this->sendReturnStatus('Ok');
    }
     ...
     * @throws Zend_Controller_Response_Exception
     */
    protected function sendReturnStatus($data = [], $httpCode = 200)
    {
