...
class DocumenttemplatesController extends Muzyka_Admin
{
    ...
    private $documenttemplatesosoby;
    ...
    public function init()
    {
        ...
        $this->registry_entities = Application_Service_Utilities::getModel('RegistryEntities');
    ...
    public function addminiAction()
    {
        ...
        $documentTemplatesInfo = $this->documenttemplates->fetchAll(['active = ?' => 1], 'name')->toArray();
        ...
        $this->view->t_data = $completedDocumentTemplatesData;
    }
    ...
    public function updateAction()
    {
            ...
            if (count($t_documenttemplatesosoby) > 0) {
                ...
                foreach ($t_documenttemplatesosoby AS $opts) {
                    if(!empty($opts->worker_id)) {
                        $rowEntry = $logic->getRegistryEntryRow($opts->worker_id);
                        
                        if (empty($rowEntry) || !empty($rowEntry->ghost)) {
                            continue;
                        }
                        
                        $select = $this->db->select()
                            ->from( 'registry_entries_entities_varchar')
                            ->where('entry_id = (?) ', $opts->worker_id)
                        ...
                        $t_options->t_persons[] = $imie;
                        $ob_worker = $imie;
                        
                        if (!empty($ob_worker)) {
                            $t_options->t_personsdata->$ob_worker = 'id' . $opts->worker_id;
                        }
                    }
                }
        ...
        $bookedNumberingSchemesIds = [];
        /** @var object $template */
        foreach ($this->documenttemplates->fetchAll() as $template) {
            if (
                $row->numberingscheme_id == $template->numberingscheme_id ||
                in_array($template->numberingscheme_id, $bookedNumberingSchemesIds)
            ) {
                continue;
            }
            $bookedNumberingSchemesIds []= $template->numberingscheme_id;
        }
        
        $conditions = array();
        if (count($bookedNumberingSchemesIds) > 0) {
            $conditions['id NOT IN (?)'] = $bookedNumberingSchemesIds;
        }
        $this->view->t_numberingschemes = $this->numberingschemes->fetchAll($conditions, 'name');

        $paginator = $this->registry_entities->getRegitryEntities();
        foreach($paginator as $k => $p){
            if(empty($p['system_name'])){
                unset($paginator[$k]);
            }
        }

        $this->view->paginator = $paginator;
        $this->view->registries = $this->registry->getAllForTypeahead();
    }
    ...
    public function saveAction()
    {
        ...
        try {
            if ($logicDocuments->isNumberingSchemaAssignedToOtherTemplate($values['id'], $values['numberingscheme_id'])) {
                throw new Exception('Schemat numeracji jest już używany w innym dokumencie');
            }
            ...
            $registry_id = $req->getParam('registry_id');
            if (!$registry_id) {
                $registry_id = $this->getRegistryId($req->getParam('content'));
            }

            if (!empty($registry_id)) {
                $params['registry_id'] = (int) $registry_id;
            }
            
            $id = $this->documenttemplates->save($params);
        ...
        foreach ($_POST AS $poster => $val) {
            ...
            if ($poster > 0) {
                try {
                    ...
                    $documents->update($t_data,'documenttemplate_id = \''.$poster.'\'');

                    $documentsPending = Application_Service_Utilities::getModel('DocumentsPending');
                    $t_data = array(
                        'status' => 0,
                    );
                    $documentsPending->update($t_data,'documenttemplate_id = \''.$poster.'\'');
                } catch (Exception $e) {
                }
            }
        }
    ...
    // comagom code start 2019.3.29
    public function getdbbasedonpermissionAction()
    {
        $deletedtId = $_POST['delete_dt_id'];
        $documenttemplate = $this->documenttemplates->getOne($deletedtId);
        $registry_id = $documenttemplate['registry_id'];
        if($registry_id == 175) {
            echo "permission";
        } else {
            echo "other";
    ...
    public function getRegistryId($content)
    {
        $registry_id = 0;
        preg_match_all ("/{(.*)}/U", $content, $matches);
        foreach ($matches[1] as $match) {
            $registry = explode(".", trim($match));
            if (count($registry) > 1 ) {
                $registryName = $registry[0];
                $registry_id = $this->registry->getRegistryIdByName($registryName);
            }
        }
        return $registry_id;
    }

    public function getOsobyIds($persons)
    {
        $osobyIds = array();
        $logins = array();
        $t_options = json_decode($persons, true);

        if(count($t_options['t_persons']) > 0){
            foreach($t_options['t_persons'] as $p){
                $p = str_replace("(", "#", $p);
                $p = str_replace(")", "#", $p);
                preg_match('/#(.*)#/', $p, $match);
                $logins[] = (string)$match[1];
            }
        }
        ...
        if (count($logins) > 0) {
            $select = $this->db->select()
                ->from( 'osoby')
                ->where('login_do_systemu IN (?) ', $logins);

            $result = $this->db->fetchAll($select);

            if ($result) {
                foreach ($result as $r) {
                    $osobyIds[$r['id']] = $r['nazwisko'] ." ". $r['imie'];
                }
            }
        }
        return $osobyIds;
    ...
    public function getRegistryEntries($registry_id)
    {
        $osobyIds = array();
        $select = $this->db->select()
    ...
    public function getRegistryEntities($registry_id)
    {
        ...
        if ($result) {
            foreach ($result as $entity) {
                if ($entity['system_name'] == "name") {
                    $registryEntities['id'] = $entity['id'];
                }
            }
        }
