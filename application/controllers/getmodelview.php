<?php include APPLICATION_PATH . '/views/templates/modals/multiform_modal.tpl'; ?>
    ...
    <div class="col-lg-12 vimal">
        <form name="f_data" id="f_data" method="POST" action="/registry-entries/save" enctype="multipart/form-data">
            <fieldset>
                ...
                <?php if ($this->data->id) {?><input type="hidden" name="id" value="{$data.id}"/> <?php }?>
                <?php if ($this->$documentTemplateId) { ?><input type="hidden" name="document_template_id" value="{$documentTemplateId}"/><?php } ?>
                ...
                <?php } ?>
                <?php foreach ($data->registry->entities as $registryEntity) { ?>
                <div style="<?php if (in_array($registryEntity->entity_id, $todisable)) { ?> pointer-events: none; <?php }?>">
                     <?php Application_Service_Entities::render($registryEntity, $data); ?>
                </div>
                <input type="hidden" name="element_{$registryEntity->id}_title" value="{$registryEntity->title}">
                <?php } ?>
                
                <?php if ($data.id && !empty($data.registry.documents_templates)) { ?>
