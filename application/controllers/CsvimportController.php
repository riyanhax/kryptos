...
//        echo "<pre>";print_r($defaultNamespace->test_xlsx);echo "</pre>";die();
...
//            $reg_HTML .='<option value="5">Dictionary</option>';
//            $reg_HTML .='<option value="65">Employees</option>';
//            $reg_HTML .='<option value="37">Building registry </option>';
            $reg_HTML .='</select>';
            $reg_HTML .='</div>';
            $reg_HTML .='</div>';
            $reg_HTML .='</div>';
...
//        echo "<pre>";print_r($html);echo "</pre>";die();
    ...
    public function insertionAction(){
        ...
        $xlxs_arr = $defaultNamespace->test_xlsx;
        $event_id = $data['event_id'];
        $map_sheet = $data['xlsx_arr'];
        $map_entity = $data['map_arr'];

        
        $result=array_intersect($xlxs_arr[0],$map_sheet);

        //$duplicaterow =  $this->duplicateEntry($xlxs_arr,$map_sheet,$event_id,$map_entity);

        //$xlxs_data = $this->removeElementWithValue($xlxs_arr, $duplicaterow);

        $xlxs_data = $xlxs_arr;
        $no_of_duplicate = count( $xlxs_arr ) - count($xlxs_data);
        $total = count($xlxs_arr);
        $new_value =  count($xlxs_data);
        ...
        if ($result_value == 1){
            $no_of_duplicate = count( $xlxs_arr ) - count($xlxs_data);
            if($no_of_duplicate == 0){
                $this->outputJson(array('status'=>1,'message'=>'Data Imported Successfully'));
            } else {
                $total = count($xlxs_arr);
                $new_value =  count($xlxs_data);
                $remove_row = $total - $no_of_duplicate;
                $this->outputJson(array('status'=>2,'message'=>'Data Imported Successfully','messageerror'=>'Auto Remove '.$remove_row.' Duplicate Row'));
            }
        }
        else        
        {
            $this->outputJson(array('status'=>3,'message'=>'Data Import Failed','messageerror'=>'No rows imported'));
        }
        die();
    }
  ...
  /*rahul create this function for duplicate entry*/

    public function duplicateEntry($xlxs_data,$map_sheet,$event_id,$map_entity){
        for($row = 1; $row<count( $xlxs_data ); $row++){
            $field = array();
            $i =0;
            foreach($xlxs_data[$row] as $key => $value){
        ...
        foreach ($datavalue as $data){
            if ($data != null && !is_numeric($data) && !preg_match("/\d{4}\-\d{2}-\d{2}/", $data)){
                $row =  $this->reg_varchar_Model->getAdapter()->select()->from('registry_entries_entities_varchar')->where('value =?',$data)->query()->fetchAll();
                if (count($row) > 0 ){
