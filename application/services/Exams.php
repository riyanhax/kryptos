...
class Application_Service_Exams
{
    ...
    public function getSession($sessionId)
    {
        ...
        shuffle($session['exam']->questions);
        foreach($session['exam']->questions as $question) {            
            shuffle($question['answers']);
        }       
        if(count($session['exam']->questions) > 25) {
            $session['exam']->questions = array_slice($session['exam']->questions, 0, 25);
        }
        return $session;
    }
