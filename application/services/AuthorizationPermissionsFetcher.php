...
class Application_Service_AuthorizationPermissionsFetcher
{
    ...
    static function registryAccessBase($registryId = null)
    {
        ...
        if ($registryId) {
            ...
            $userId = Application_Service_Authorization::getInstance()->getUserId();
            $entry = $registryModel->getOne($registryId);
        }
       $user = Application_Service_Authorization::getInstance()->getUser();
       $rights = json_decode($user['rightsPermissions']);


       return Application_Service_Authorization::getInstance()->isSuperAdmin() || 
       isset($rights->{'perm/'.$registryId}) && $rights->{'perm/'.$registryId} ? true : false; 
    }
    ...
    static function registryAccessById($registryId = null, $entryId = null, $mode = 'admin')
    {
        ...
        return self::registryAccessBase($registryId);
        /*$assigneesModel = Application_Service_Utilities::getModel('RegistryAssignees');
        $conditions = [
            'user_id = ?' => $userId,
