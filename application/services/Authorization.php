...
class Application_Service_Authorization
{
    ...
    protected function checkGranted($permissionString, $params)
    {
        ...
        if ($permission[0] === 'perm') {
            if (!$this->isGrantedPerm($permission, $params)) {
                return false;
            }
            return true;
        }
    ...
    private function isGrantedNode($permission, $params = null)
    {
        ...
        if (isset($this->moduleSettings['nodes'][$controller])) {
            ...
            if (!empty($requiredPermissions['roles'])) {
                $role = null;
                ...
                switch (true) {
                    case self::isAdmin():
                        $role = 'admin';
                        break;
                    case self::isSuperAdmin():
                        $role = 'superadmin';
                        break;
                }
                
                if (!empty($requiredPermissions['roles'][$role])) {
                    return true;
                }
            }
    ...
    private function fetchPermissions($fetchFunctions, $params = null)
    {
     ...
     private function isGrantedPerm($permission, $params=null)
    {
        ...
        if (sizeof($permission) > 2) {
            $permissionString = $permission[0] . '/' . $permission[1];
        } else {
        ...
        if(isset($params['author']) && is_numeric($permission[1]))
        {
            
            if(empty($user['rightsPermissions']))
            {
                $accTypeObj = $adminLink->getTypeByLogin($user['login']);
                $accType = 'mini';
                 if (isset($accTypeObj['type'])) {
                    $accType = $accTypeObj['type'];
                }
                $user['rightsPermissions'] = $this->rightsPermissionsModel->getBytype($accType);
            }
            $rights_permissions = json_decode($user['rightsPermissions']);
            $all_perms = $permission[0].'/'.$permission[1].'/';

            if(isset($rights_permissions->{$all_perms.'admin'}) && $rights_permissions->{$all_perms.'admin'})
            {
                
                return true;
            }
            //echo "<li>".$user['id'];
            //echo "<li>".$params['author'];
            //echo "<li>".$permissionString;
            switch ($permission[2]) {
                case 'write.all':
                    return self::isSuperAdmin() || (isset($rights_permissions->{$permissionString}) && $rights_permissions->{$permissionString}) ? true : false;
                break;
                case 'write.my':
                        return self::isSuperAdmin() || ($user['id'] == $params['author'] && isset($rights_permissions->{$permissionString}) && $rights_permissions->{$permissionString}) ? true : false;
                break;
                case 'read.all':
                    return self::isSuperAdmin() || (isset($rights_permissions->{$permissionString}) && $rights_permissions->{$permissionString}) ? true : false;
                break;
                case 'read.my':
                    if(isset($rights_permissions->{$all_perms.'read.all'}) && $rights_permissions->{$all_perms.'read.all'})
                    {
                        return true;
                    }
                        return self::isSuperAdmin() || (isset($rights_permissions->{$permissionString}) && $rights_permissions->{$permissionString}) ? true : false;
            ...
            //return self::isSuperAdmin() || $isAuthor || isset($rights_permissions->{$permissionString}) && $rights_permissions->{$permissionString} ? true : false;
            return self::isSuperAdmin() || isset($rights_permissions->{$permissionString}) && $rights_permissions->{$permissionString} ? true : false;
    ...
    public static function isCompanyConfirmation()
    {
        $user = self::getInstance()->getUser();

        return (bool) $user['company_confirmation'];
    }
...
END;
    ...
    public function isEmployee($idEmployee)
    {
	$adminLinkModel = Application_Service_Utilities::getModel('AdminLink');

	$osobyModel = Application_Service_Utilities::getModel('Osoby');

	$login = $osobyModel->getLoginById($idEmployee);

	return $adminLinkModel->isEmployee(Application_Service_Authorization::getInstance()->getUserId(), $login);
    }
