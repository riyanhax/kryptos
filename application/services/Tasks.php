...
class Application_Service_Tasks
{
    ...
    const TRIGGER_TYPE_AUTOMATICALLY_WHEN_NEW_DOCUMENT = 6;
    ...
    /**
     * @return Application_Service_Tasks
     */
        ...
        return array(
            ...
            self::TYPE_DOCUMENT => array(
                ...
                'triggerTypes' => array(
                    ...
                    self::TRIGGER_TYPE_MULTI_DAILY,
                    self::TRIGGER_TYPE_SINGLE_IMMEDIATELY,
                    self::TRIGGER_TYPE_AUTOMATICALLY_WHEN_NEW_DOCUMENT,
                ),
                'is_visible' => true,
            ),
            ...
            self::TYPE_PROPOSAL_EMPLOYEE_ADD_EMPLOYEE_ACCESS => array(
                ...
                'is_visible' => false,
            self::TYPE_REGISTRY => array(
                    'id' => self::TYPE_REGISTRY,
                    'name' => 'Rejestr',
                    'triggerTypes' => array(
                        self::TRIGGER_TYPE_SINGLE,
                        self::TRIGGER_TYPE_MULTI_MONTHLY,
...
/*            self::TYPE_SURVEY => array(
    ...
    public function getTriggerTypes()
    {
        return array(
            ...
            array('id' => self::TRIGGER_TYPE_AUTOMATICALLY_WHEN_NEW_DOCUMENT, 'name' => 'uruchom zadanie za każdym razem, gdy nowy aktywny dokument'),
    ...
    public function eventNextRoundTaskCreate($document, $osobaId)
    {
        $task = $this->tasksModel->findOneBy(array(
            'type = ?' => self::TYPE_DOCUMENT,
            'object_id = ?' => $document['documenttemplate_id'],
            'status = ?' => 1,
            'trigger_type' => self::TRIGGER_TYPE_AUTOMATICALLY_WHEN_NEW_DOCUMENT,
        ));

        if (!empty($task) && !empty($document['active'])) {
            // jeśli jest zadanie przypisane do tego szablonu dokumentu
            $this->createStorageTaskSimple($task, $document['id'], $osobaId, date('Y-m-d'));
        }
    }
    ...
    public function createStorageTask($data)
    {
        ...
        if (!empty($data['send_notification_sms'])) {       
            $task = $this->tasksModel->getOne(['id = ?' => $data['task_id']]);
            $user = $this->osobyModel->getOne(['o.id = ?' => $data['user_id']]);
            $this->notificationsService->scheduleSMS([
                'type' => Application_Service_Notifications::TYPE_TASK,
                'user_id' => $data['user_id'],
                'title' => 'Kryptos - nowe zadanie',
                'template' => 'task_new',
    ...
    public function createStorageTaskSimple($task, $objectId, $userId, $currentDate = null, $data = [])
    {
        ...
        $data = array_merge([
            ...
            'title' => $task['title'],
            'description' => $task['message_template'],
            'object_id' => $objectId,
    ...
    public function confirmTask($storageTaskId, $viewDate = null)
    {
        ...
        {
            $newtask = $this->tasksModel->findOne($task->parent_id);
            if(!empty($newtask))
            {
                $this->assingNewTaskToUser($newtask, $storageTask['user_id']);
            }
        }  
            ...
            switch ($task['type']) {
                case self::TYPE_DOCUMENT:
                    ...
                    foreach($usersIds as $userId) {
                        if($this->osobyModel->getWorkerIdFromUserId($userId) != null) {
                            $workerIds[] = $this->osobyModel->getWorkerIdFromUserId($userId);
                        }
                    }
                    ...
                    foreach ($documents as $document) {
                        $this->createStorageTaskSimple($task, $document['id'], $this->osobyModel->getUserIdFromWorkerId($document['d.worker_id']), $currentDate);
                    }
                    break;
    ...
    public function assingNewTaskToUser($newtask, $user_id)
    {
        $currentDate = date('Y-m-d');
        ...
        switch ($newtask['type']) {
            case self::TYPE_DOCUMENT:
                $documentsModel = Application_Service_Utilities::getModel('Documents');
                $documents = $documentsModel->getList(array(
                    'd.osoba_id IN (?)' => $user_id,
                    'd.documenttemplate_id = ?' => $newtask['object_id'],
                    'd.active = ?' => Application_Service_Documents::VERSION_OBLIGATORY,
                ));

                foreach ($documents as $document) {
                    $this->createStorageTaskSimple($newtask, $document['id'], $document['osoba_id'], $currentDate);
                }
                break;
            case self::TYPE_DOCUMENT_VERSIONED:
                $actualVersion = $this->documentsVersionedVersionsModel->findOneBy(array(
                    'document_id = ?' => $newtask['object_id'],
                    'status = ?' => Application_Model_DocumentsVersionedVersions::VERSION_ACTUAL,
                ));

                if ($actualVersion) {
                    $this->createStorageTaskSimple($newtask, $actualVersion['id'], $user_id, date('Y-m-d'));
                }
                break;
                
             case self::TYPE_COURSE:
                    $session = $this->coursesService->createSession($newtask['object_id'], $user_id);
                    $this->createStorageTaskSimple($newtask, $session->id, $user_id, $currentDate);
                    break;
            case self::TYPE_TICKET:
                case self::TYPE_PROCEDURE_NEW_USER_SET_PERMISSIONS:
                    // do nothing
                    break;
                default:
                    $this->createStorageTaskSimple($newtask, null, $user_id, $currentDate);
        }
    }
    public function assingTaskToOsoba($osoba_id)
    {
        ...
        if (!empty($tasks)) {
            foreach ($tasks as $task) {
                $this->assingNewTaskToUser($task, $osoba_id);
                if ($task['users_type'] == 2) 
                {
                    $taskUsersModel->delete(array('task_id = ?' => $task['id'], 'user_id = ?' => $osoba_id));
