...
class Application_Service_HtmlHelper
{
    ...
    public static function renderElement($params)
    {
        ...
        switch ($tag) {
            case "bs.typeahead":
                ...
                break;
            case 'bs.hidden':
                $defaultTemplate = 'hidden.';
                break;
            case 'bs.paragraph':
                $defaultTemplate = 'paragraph.';
                break;
            case 'bs.moreInfo':
                $defaultTemplate = 'moreInfo.';
                break;
            case 'bs.header':
            ...
            case 'bs.number':
                ...
                break;
            case 'bs.radioGroup':
                $defaultTemplate = 'ragiogroup.';
                break;
            case 'bs.button':
            case 'bs.autocomplete':
            case "bs.varchar":
            ...
            case "bs.dropzone":
                ...
                break;
            case 'bs.checkboxGroup':
            case "bs.checkboxes":
                $defaultTemplate = 'checkboxes.';
                break;
            case "bs.googlemap":
                $defaultTemplate = 'googlemap.';
                break;
            case "bs.datagrid":
                $defaultTemplate = 'datagrid.';
                break;
            case "bs.rating":
                $defaultTemplate = 'rating.';
                break;
            case "bs.signature":
                $defaultTemplate = 'signature.';
                break;
            case "bs.relationshipMatrix":
            case "bs.relationshipMatrixMultiple":
            case "bs.relationshipMatrixDynamic":
            case "bs.relationshipMatrixExtra":
                $defaultTemplate = substr($tag, 3) . '.';
                $attributes['value'] = $params['attributes']['value'];
                break;
            case "bs.smartRadioGroup":
                $defaultTemplate = 'smartRadioGroup.';
                break;
            case "bs.smartMultiSelect":
                $defaultTemplate = 'smartMultiSelect.';
                break;
            case "bs.hyperlink":
                $defaultTemplate = 'hyperlink.';
                break;
        }
        ...
        if ($params['attributes']['colNo']=='2') {
            $wrapper['class'] []= 'col-md-6';
        }
        if (!empty($params['attributes']['multiformName'])) {
            $wrapper['class'] []= $params['attributes']['multiformName'];
        }
    ...
    /*render view*/
    public static function renderElementText($params)
    {   
        ...
        if (!empty($wrapper_attributes)) {
            $wrapper['attributes_string'] = self::getAttributesStringText($wrapper_attributes);
        }
        $wrapper['class'] = implode(' ', $wrapper['class']);

        $template = sprintf('_reuse/text/%selement.html', !empty($params['template']) ? $params['template'] : $defaultTemplate);
        return Application_Service_Utilities::getInstance()->renderViewText($template, compact('tag', 'attributes', 'wrapper', 'innerHtml', 'isSingle', 'params'));
    }

    public static function getAttributesStringText($array)
    {
        $result = [];
        foreach ($array as $name => $value) {
            $result[] = sprintf('%s="%s"', $name, htmlspecialchars($value, ENT_COMPAT));
