...
class Application_Model_Tasks extends Muzyka_DataModel
{
    ...
    public function getTaskByObjectId($id)
    {
        $sql = $this->select()
            ->where('object_id = (?)', $id)->where('status = (?)', 1);
    ...
    public function save($data)
    {
        ...
        if (!empty($data['task_users'])) {
            foreach ($data['task_users'] as $userId => $isSelected) {
                if ($isSelected === '1' || $data['users_type'] == 1) {
...
*/
    ...
    {
        $select = $this->_db->select()
            ->from(array('t' => $this->_name), array('id', 'name'=>'title'))
            ->where('t.type < 1000')
            ->order('t.title ASC');
    ...
    public function addTaskAutomatically($param)
    {
        $params = Array(
            'module' => 'default',
            'status' => 1,
            ...
            'type' => 1,
            'trigger_type' => 1,
            'trigger_mode_1' => Array(
                                    'date' => date("Y-m-d")
                                ),
            'activate_before_days' => 1,
            'title' => $param['name'],
            'author_osoba_id' => 4,
            'send_notification_sms' => 1,
            'send_notification_message' => 1,
            'send_notification_email' => 1,
            'task-employees_length' => 10,
            'task_users' => Array
            (
                '2' => 0,
                '3' => 0,
