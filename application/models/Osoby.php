...
class Application_Model_Osoby extends Muzyka_DataModel
{
    ...
    const USER_TYPE_EXPERT = 1;
    const USER_TYPE_PRO = 2;
    const USER_TYPE_MINI = 3;
    ...
    public function getOne($id = [], $required = false)
    {
        $sql = $this->select()
            ->where('id = ?', $id);
    ...
    public function save($data)
    {
        ...
        $row->type_of_user = $data['type_of_user'];
        $row->month_of_birthday = $data['month_of_birthday'];
        $row->day_of_birthday = $data['day_of_birthday'];
        $row->assigned_worker_id = $data['assigned_worker_id'];
        ...
        if (!empty($data['phone'])) {
            $data['telefon_stacjonarny'] = $data['telefon_komorkowy'] = $data['phone'];
        }
        ...
        $row->birth_day = $data['day'];
        $row->birth_month = $data['month'];
        ...
        if (Application_Service_Authorization::isGranted('perm/osoby/set-permissions')) {
            if (!empty($data['rightsPermissions']) && is_string($data['rightsPermissions'])) {
                $row->rightsPermissions = $data['rightsPermissions'];
            } else {
                $row->rightsPermissions = '';
            }
        } else {
            $row->rightsPermissions = '';
        }
        ...
        $id = $row->save();
        /*if (empty($data['id'])) {
            $this->taskService = Application_Service_Tasks::getInstance();
            $this->taskService->assingTaskToOsoba($row->id);
        }*/
    ...
    public function getAllUserByLogin($login)
    {

        $sql = $this->select()
            ->where('login_do_systemu IN (?)', $login);
        $result = $this->fetchAll($sql);
	    return $result->toArray();
    }

    public function getWorkerIdFromUserId($user_id)
    {
        $sql = $this->select()
            ->where('id = ?', $user_id);
    ...
    public function getUserIdFromWorkerId($worker_id)
    {
        $sql = $this->select()
    ...
    public function getAssignedWorkerIds(array $excludes = [])
    {

        $sql = $this->select()
            ->where('assigned_worker_id')->Where('usunieta = ?', 0);
        if ($excludes) {
            $sql->where('assigned_worker_id NOT IN (?)', $excludes);
        }
        return array_filter(array_map(function($data){
            return $data['assigned_worker_id'] ?: -1;
        }, $this->fetchAll($sql)->toArray()));
    }

    public function generateUserLogin($data)
    {
        $nazwisko = !empty($data['nazwisko']) ? $this->removePL(trim($data['nazwisko'])) : '';
        $imie = !empty($data['nazwisko']) ? $this->removePL(trim($data['imie'])) : '';

        $loginBase = mb_strtoupper(mb_substr($imie, 0, 2) . mb_substr($nazwisko, 0, 2));
        if (!$loginBase) {
            if (!empty($data['email'])) {
                $loginBase = mb_strtoupper(mb_substr(hash('sha256', serialize($data)), 0, 5));
            }
        }
        $number = '';
    ...
    {
	$sql = $this->select('login_do_systemu')
            ->where('id = ?', $id);

        $data = $this->fetchRow($sql);
	return $data['login_do_systemu'];
    }
    ...
    public function countActive($users)
    {
	$sql = $this->select()
	    ->where('login_do_systemu IN (?)', $users);
