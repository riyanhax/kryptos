...
class Application_Model_Registry extends Muzyka_DataModel
{
    ...
    public function save($data)
    {
        $defaultData = [
            ...
            'is_locked' => 0,
            'is_visible' => 1,
            'steps_mode' => self::STEPS_MODE_WIZARD,
        ];
    ...
    public function getRegistryById($parmId)
    {
    ...
    public function cloneRegistrySettings($registryId, $cloneId)
    {
        ...
        $this->_db->query(sprintf('INSERT INTO registry_entities SELECT null, %d, entity_id, system_name, null, title, is_multiple, null, config, null, null, null, `order`, NOW(), null FROM registry_entities WHERE registry_id = %d', $registryId, $cloneId));
    ...
    public function getRegistryByTitle($title)
    {
        return $this->getAdapter()
            ->select()
            ->from($this->_name)
