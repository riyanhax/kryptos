...
class Application_Model_RegistryEntities extends Muzyka_DataModel
{
    ...
    public function save($data)
    {
        ...
        $config = new stdClass();
        if (!empty($data['config_data']) && !empty($row->config)) {
    ...
    public function getEntitesWithoutAccordion($regId)
    {
	 ...
	 public function updateEntityForAccordion($id,$data){
      ...
      public function getRegistryIdByRegistryEntityId($id)
      {
            return $this ->getAdapter()
            ->select()
            ->from($this->_name)
    ...
    public function getIdByUniqueName($registyId, $name){
        return $this ->getAdapter()
            ->select()
            ->from($this->_name)
            ->where('registry_id=?', $registyId)
            ->where('name=?', $name)
            ->query()
            ->fetchColumn();
    }
    ...
    public function getRemovedEntities($registry_id, $existing_ids)
    {
       $result = $this->_db->query(sprintf('SELECT * FROM `registry_entities` where `registry_id` = "'.$registry_id.'" AND id NOT IN ('.implode(',', $existing_ids).')'))->fetchAll(PDO::FETCH_ASSOC);
       return $result;
    }

    public function getEntitiesByRegistryId($registry_id)
    {
       $result = $this->_db->query(sprintf('SELECT * FROM `registry_entities` where `registry_id` = "'.$registry_id.'"'))->fetchAll(PDO::FETCH_ASSOC);
       return $result;
    }

    public function getRegitryEntities()
    {
        $result = $this->_db->query(sprintf('SELECT re.id, re.title, re.system_name, r.title as r_title, r.id as r_id  FROM `registry_entities` re INNER JOIN `registry` r ON re.registry_id=r.id'))->fetchAll(PDO::FETCH_ASSOC);
        return $result;
    }
