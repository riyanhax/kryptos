...
class Application_Model_StorageTasks extends Muzyka_DataModel
{
    ...
    public function checkExisting($object_id,$task_id,$user_id)
    {
        $sql = $this->select()
            ->where('task_id = (?)', $task_id)->where('user_id = (?)', $user_id);
    ...
    public function removeByObjectId($object_id)
    {
        $sql = $this->select()
            ->where('object_id = (?)', $object_id);
        $rows = $this->fetchAll($sql)->toArray();
        foreach ($rows as $key => $row) {
            $row = $this->requestObject($row['id']);
            $row->delete();
            $this->addLog($this->_name, $row->toArray(), __METHOD__);
        }
    }
    ...
    public function getSignatures($conditions, $limit = null, $order = null)
    {
        $select = $this->_db->select()
            ...
            ->where('st.status = ?', 1)
             ->joinLeft(array('o' => 'osoby'), 'st.user_id = o.id', array('imie', 'nazwisko', 'email', 'osoba_id' => 'id'))
