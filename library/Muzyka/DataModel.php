...
abstract class Muzyka_DataModel extends Zend_Db_Table_Abstract
{
    ...
    {
        $sql = $this->select();
        $sql->order('insert_date DESC');
        return $this->fetchAll($sql);
    }
    public function getAllAuditAction($field, $registry_id)
    {  
        $sql = $this->select();
        $sql->where($field . ' = ?', $registry_id);
        return $this->fetchAll($sql);
    }
    public function getAllLogAction($field, $registry_id)
    {  
        $sql = $this->select();
        $sql->order('insert_date DESC');
        $sql->where($field . ' = ?', $registry_id);
