...
(function($) {
    $.fn.sortableLists = function(options) {
       ...
       /*  return this.on('mousedown', function(e) {
            ...
            console.log('current startDrag', el.data());

            var children = (el.children('ul')).children('li');
            console.log('children count', children.length);

            // prevent more than 3-level by storm
            if (children.length) {
                return;
            }
      ...
      function endDrag(e) {
            ...
            cEl.el.animate({ left: offset.left - $(window).scrollLeft() - state.cEl.mL, top: offset.top - $(window).scrollTop() - state.cEl.mT }, 250,
                function() // complete callback
                {
                    ...
                    console.log('current', cEl.el.data());
                    var data = cEl.el.data();

                    console.log('count', hintWrapperNode.length);
                    if (hintWrapperNode.length) {
                        var item = hintWrapperNode.prev('div').first();
                        var parent_data = $(item).closest('li').data();
                        console.log('parent data', data);
						  var order = new Array();
							$('ul#myEditor li').each(function() {
							order.push($(this).attr("id"));
							});
							updateorder(order);
						//alert(data);
                        updateMenuData(data, parent_data);

                    } else {
                        var node = ((cEl.el).parent('ul')).parent('li').first();
                        var node_li = node.closest('li');
                        var parent_data = node_li.data();
                        console.log('parent data----', data);
						//e.preventDefault();
...
*/
        function updateMenuData(data, parent_data) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    parent_data: JSON.stringify(parent_data)
                },
                async: true,
...
function MenuEditor(idSelector, options) {
    ...
    /*start menu buttons  by rahul*/

    $(document).on('click', '.btnRemove', function(e) {
        e.preventDefault();
        var data = [];
        var list = $(this).closest('ul');
        var parent_data = $(this).closest('li').data();
        console.log('parent', $(this).closest('li').data());
        data.push(parent_data);
        var ul_children = ($(this).closest('li').first()).children('ul');
        // console.log('list count', ul_children.length);
        var label = $(this).closest('li').data('label');
        if(ul_children.length > 0){
            var dialog = $('<p>This ' + label + ' will be deleted. <br />Are you sure ?</p>').dialog({
            ...
            function parentchild(){
                ...
                if (ul_children.length) {
                    var li_children = ul_children.children('li');
                    li_children.each(function() {
                        var child_data = $(this).data();
                        console.log('tt data', child_data);
                        data.push(child_data);
                    })
                }
                var option = 'parentchild';
                removeData(data,option);
            ...
            if (confirm("This " + label + " will be deleted. Are you sure?")) {
                var list = $(this).closest('ul');
                var parent_data = $(this).closest('li').data();
                data.push(parent_data);
    ...
    function removeData(data,option) {
        var baseUrl = window.location.protocol + '//' + window.location.host + window.location.pathname;
        $.ajax({
            dataType: 'json',
            url: baseUrl + '/remove-data',
            type: 'POST',
            data: {
                data: JSON.stringify(data),
                option : option
    ...
    $main.on('click', '.btnUp', function(e) {
        ...
        var child_id = $(this).closest('li').attr('ids');
	
        var parentid = $li.prev('li').before($li).attr('ids');
    ...
    $main.on('click', '.btnDown', function(e) {
        ...
        var $li = $(this).closest('li');
        var child_id = $(this).closest('li').attr('ids');
        var parentid = $li.next('li').after($li).attr('ids');

        var baseUrl = $('#base_url').val();
        $.ajax({
            dataType: 'json',
            url:        baseUrl + '/move-down',
            type:       'POST',
            data: {
                child_id: child_id,
                parentid: parentid
    ...
    $main.on('click', '.btnOut', function(e) {
        ...
        var content_out = $(this).closest('li').attr('ids');
        var parentid = $li.closest('ul').closest('li').attr('ids');
    ...
    $main.on('click', '.btnIn', function(e) {
        ...
        var child_id = $(this).closest('li').attr('ids');
        var $prev = $li.prev('li');
        var parent_id = $li.prev('li').attr('ids');
    ...
    this.update = function() {
        ...
        $cEl.find('span.txt').first().text($cEl.data('label'));
        resetForm();
    };
    ...
    this.add = function() {
        ...
        var textItem = $('<span>').addClass('txt').text(data.label);
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function(k, v) {
            ...
            var itemObject = { label: "", path: "", icon: "empty", target: "_self", rel: "" };
            var v2 = $.extend({}, v);
            if (isParent) {
                delete v2['children'];
            }
            ...
            var $span = $('<span>').addClass('txt').append(v.label);
    ...
    function TButtonGroup() {
        ...
        var $btnUp = TButton({ title: 'move up', classCss: 'btn btn-default btn-xs btnUp btnMove', text: '<i class="glyphicon glyphicon-chevron-up clickable"></i>' });
        var $btnDown = TButton({ title: 'move down', classCss: 'btn btn-default btn-xs btnDown btnMove', text: '<i class="glyphicon glyphicon-chevron-down clickable"></i>' });
        var $btnOut = TButton({ title: 'move out', classCss: 'btn btn-default btn-xs btnOut btnMove', text: '<i class="glyphicon glyphicon-save clickable"></i>' });
        var $btnIn = TButton({ title: 'move in', classCss: 'btn btn-default btn-xs btnIn btnMove', text: '<i class="glyphicon glyphicon-export clickable"></i>' });
        $divbtn.append($btnUp).append($btnDown).append($btnIn).append($btnOut).append($btnEdit).append($btnRemv);
        return $divbtn;
    }

    function TButton(attr) {
        return $("<a>").addClass(attr.classCss).addClass('clickable').attr("path", "#").attr("title", attr.title).html(attr.text);
    }
...
/*for register menu editor by rahul*/

function MenuEditor2(idSelector, options) {
    var $main = $("#" + idSelector);
    var settings = {
        labelEdit: '<i class="glyphicon glyphicon-edit clickable"></i>',
        labelRemove: '<i class="glyphicon glyphicon-remove clickable"></i>',
    ...
    function TButtonGroup(v) {
        var $divbtn = $('<div>').addClass('pull-right');
        var $btnEdit = TButton({ classCss: 'btnEdit ', data: v});
        $divbtn.append($btnEdit);
        return $divbtn;
    }

    function TButton(attr) {
        var value = attr.data.id;
        // return $("<a>").addClass(attr.classCss).addClass('clickable').attr("path", "#").attr("title", attr.title).html(attr.text);
        $data =  $('<label class="myCheckbox">').addClass('clickable');
        $data.append($('<input type="checkbox" name="registry" id="'+ value +'" value="'+ value +'">').addClass('clickable'));
        $data.append($('<span></span>').addClass('clickable'));
        return $data;
    }
