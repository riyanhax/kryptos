...
enableJsCheckboxes = function () {
    var checkboxSpoofElementClick = function () {
        var tg = $(this),
                checkbox = tg.attr("data-target-id")
                ? $('input[name="' + tg.attr("data-target-id") + '"]')
                : tg.find("input");
    ...
    var checkboxChange = function () {
        var tg = $(
                '.js-checkbox-spoof-element[data-target-id="' + this.name + '"]'
                );
    ...
    $(".action-select-all")
            ...
            .on("click", function () {
                ...
                if (tg.hasClass("checked")) {
                    ...
                    tr.find(".js-checkbox-from-container.checked").each(function () {
                    ...
                    tr.find(".js-checkbox-from-container:not(.checked)").each(function () {
                        var event = $.Event("click", {
                            target: this
                        });
                        tr.trigger(event);
            ...
            .each(function () {
                var tg = $(this),
                        targetSize = tg
                        .closest("tr")
                        .find(".js-checkbox, .js-checkbox-from-container")
                        .size();

                if (
                        tg
                        .closest("tr")
                        .find(".js-checkbox.checked, .js-checkbox-from-container.checked")
                        .size() === targetSize
                        ) {
                    tg.addClass("checked");
                }
...
enableDropzone = function (injectParams) {
    ...
    if (
            dropzoneElement.size() < 1 ||
            dropzoneElement.hasClass("processed-dropzone")
            ) {
        return;
    }
    var params = {};
    ...
    dropzoneElement.dropzone(params).addClass("processed-dropzone");
...
enableUploadList = function () {
    ...
    $(".courses-slide-list .slide")
            ...
            .each(function () {
                ...
                tg.append(
                        '<span class="image-delete-button glyphicon glyphicon-remove"></span>'
...
var dial = {
            ...
            setSummaryView: function () {
                ...
                if (valuesStorage["t_persons"].length == 0) {
                    viewContainer.append(
                            '<div class="alert alert-info"><i class="fa fa-info fa-fw"></i> &nbsp;<i>Nie wybrano osób. Szablon będzie aktywny dla wszystkich osób.</i></div>'
                            );
                }
                ...
                $.each(valuesStorage["t_persons"], function (i2, v2) {
                    viewContainer.append(
                            ...
                            "onclick=\"activeFields = 0; deletePerson('" +
                            valuesStorage["t_personsdata"][v2] +
                            "','" +
                            v2 +
                            "');\"" +
                            ">" +
    ...
    prepareStarter: function (callerButton, dialogClass) {
        var url = callerButton.attr("data-dial-url"),
                ...
                processFn = callerButton.attr("data-dial-process-fn")
                ? $.proxy(
                        window[callerButton.attr("data-dial-process-fn")],
                        callerButton
                        )
                : $.proxy(dial.defaultDialogProcess, callerButton),
                readyFn = callerButton.attr("data-dial-ready-fn")
                ? $.proxy(window[callerButton.attr("data-dial-ready-fn")], callerButton)
                : $.noop,
                dialModel = callerButton.attr("data-dial-model")
                ? $.proxy(window[callerButton.attr("data-dial-model")], callerButton)
                : $.noop,
                dataProcessFn = callerButton.attr("data-dial-data-process-fn")
                ? $.proxy(
                        window[callerButton.attr("data-dial-data-process-fn")],
                        callerButton
                        )
                : $.noop,
                data = {
        ...
        var openDialog = function () {
            ...
            if (newDialog) {
                ...
                if (choosedUrl) {
                    showDial(
                            urlAddHintValue(choosedUrl),
                            dialClass ? dialClass : "modal-lg",
                            ""
                            );
                }
            }
        ...
        var urlAddHintValue = function (url) {
            var hintValue = callerButton
                    .closest(".input-group")
                    .find(".typeaheadElement")
                    .val();
            if (!url.match(/\?/)) {
                url += "?";
            } else {
    ...
    defaultDialogProcess: function (result) {
        var targetElement = $(this.attr("data-target-element")),
                ...
                targetElementId = $(this.attr("data-target-element-id"));
        if (typeof result === "object") {
            if (sourceVariableName) {
                //ealier code was not working properly.
                window[sourceVariableName] = result;
            }
            targetElement.val(result.id).change();
            targetElementId.val(result.displayName);
        } else {
            targetElement.val(result).change();
            targetElementId.val(result);
        }
...
newDialogSuccess = function (result) {
    ...
    if (
        ...
        if (dialogBody.find(".footer-actions").size()) {
            dialog
                    .find(".modal-footer")
                    .html(dialogBody.find(".footer-actions").html());
        ...
        do {
            $.each(timeouter.runOnExit, function (roeIndex, storageConfig) {
                ...
                $.each(timeouter.runOnExit, function (idx, cfg) {
                    var dependentIdx = cfg.unloadBefore.indexOf(
                ...
                $.each(timeouter.storageTimeout[storageConfig.storageName], function (
                        storageId,
                        timeout
                        ) {
                    if (timeout) {
                ...
                timeouter.runOnExit.length > timeouter.exitCompleted.length ||
                ++l > 1000
                );
...
typeaheadInit = function () {
    ...
    $(".typeaheadElement")
            ...
            .each(function () {
                var tg = $(this),
                        sourceFn = tg.attr("data-source-function")
                        ? window[tg.attr("data-source-function")]
                        : $.proxy(
                                getSourceFromStorage,
                                window[tg.attr("data-source-variable")]
                                ),
                        targetElement = $(tg.attr("data-target-element")),
                        relativeElement = tg.attr("data-relative-element")
                        ? $(tg.attr("data-relative-element"))
                        : null,
                        afterSelectFn = tg.attr("data-after-select-fn")
                        ? window[tg.attr("data-after-select-fn")]
                        : $.noop;
                        ...
                        .change(function () {
                            ...
                            if (current) {
                                $(dependentTypeaheads).each(function () {
                                    ...
                                    if (
                                            targetElement.is(
                                                    dependentTypeahead.attr("data-relative-element")
                                                    )
                                            ) {
                                ...
                                if (current.name.replace(/[\r\n]+/, "") == tg.val()) {
                                    ...
                                    if (relativeElement && !relativeElement.val()) {
                                        relativeElement.val(current.relation).change();
                                    }
                ...
                if (targetElement) {
                    targetElement.on("change", setNameById).change();
                }
            })
...
wizardInit = function () {
    $(".wizardElement")
            ...
            .each(function () {
                var navigation =
                        this.hasAttribute("data-navigation") &&
                        this.getAttribute("data-navigation") === "false"
                        ? false
                        : true;
                $(this).easyWizard({
                    stepsText: "{n} {t}",
            ...
            .each(function () {
                ...
                wizardElement.easyWizard({
                    ...
                    before: function (wizardObj, currentStepObj, nextStepObj) {
                        var selectedAnswer = currentStepObj
                                .find("input[type=checkbox]:checked")
                                .size()
                                ? true
                                : false,
                                stepBackward =
                                currentStepObj.prevAll().size() > nextStepObj.prevAll().size(),
                                step = nextStepObj.prevAll().size();

                        if (stepBackward || selectedAnswer || timeOverFlag) {
                            updateProgress(step);
...
ajaxFormInit = function () {
    $(".ajax-form")
            ...
            .each(function () {
                var form = $(this),
                        processFn = this.hasAttribute("data-process-fn")
                        ? $.proxy(window[this.getAttribute("data-process-fn")], form)
                        : $.noop,
                        submitFn = this.hasAttribute("data-submit-fn")
                        ? $.proxy(window[this.getAttribute("data-submit-fn")], form)
                        : null;
                ...
                if (submitFn) {
                    ...
                    form.on("submit", function (e) {
                        ...
                        $.ajax({
                            ...
                            data: submitData,
                            success: $.proxy(defaultAjaxResponseHandler, {
                                processFn: processFn
                            })
                        });
                    });
...
defaultAjaxResponseHandler = function (result) {
    var processFn =
            typeof this.processFn !== "undefined" ? this.processFn : $.noop;
    ...
    if (tg.closest("form").size()) {
        $(tg.closest("form").get(0).elements).each(function () {
            if (
                    check &&
                    $(this).is(
                    "input[type=text], input[type=radio], input[type=checkbox], input[type=file], textarea, select"
                    )
                    ) {
                found = $(this);
                check = false;
    ...
    $(".transfer-value")
            ...
            .each(function () {
                ...
                tg.on("click change ifChanged ifChecked", function () {
                    ...
                    if (
                            tg.is(":checkbox") &&
                            tg.attr("data-value-on") &&
                            tg.attr("data-value-off")
                            ) {
                        if (tg.is(":checked")) {
...
universalRemoveRowInit = function () {
    $(".remove-row")
            ...
            .each(function () {
                $(this).on("click", function () {
                    var tg = $(this),
                            beforeRemove = tg.attr("data-before-remove-fn")
                            ? $.proxy(window[tg.attr("data-before-remove-fn")], tg)
                            : $.noop,
...
tableMaxInit = function () {
    var tableEnableFilters = function (table) {
        ...
        var thead = table.find("thead"),
                ...
                filtersRow = $("<tr>"),
                filtersCols = new Array(ths.size() + 1).join(
                '<td class="filter-row"></td>'
                );

        filtersRow.addClass("filters-row").append(filtersCols);
    ...
    var tableSetFilter = function (th, filter) {
        ...
        filter.addClass("form-control").addClass("filter-item");
    ...
    var retreiveColumnValues = function (th, itemMode) {
        var table = th.closest("table"),
                ...
                realValues = {},
                result =
                '<option value="">wybierz</option><option value="">wszystkie</option>',
                uniqueNames = [],
                key;
        ...
        table.find("tbody tr").each(function () {
            ...
            if (itemMode) {
                column.find(".select-item").each(function () {
                    var key = this.hasAttribute("data-select-value")
                            ? this.getAttribute("data-select-value")
                            : $(this)
                            .text()
                            .trim();
                    key = key.toUpperCase();
                    values[key] = true;
                    realValues[key] = this.hasAttribute("data-value")
    ...
    var initDatatableEvent = function (e, settings) {
        ...
        if (operationsFooter.is(".table-operations-footer")) {
            operationsFooter
                    ...
                    .on("click", function (e) {
                        var tg = $(this),
                                triggerFn = this.hasAttribute("data-trigger-fn")
                                ? window[this.getAttribute("data-trigger-fn")]
                                : null,
                                actionUrl = this.hasAttribute("data-action-url")
                                ? this.getAttribute("data-action-url")
                                : null;

                        if (
                                table.find(":checkbox:checked").size() === 0 &&
                                $(".js-checkbox-target[value=1]").size() === 0
                                ) {
                            return false;
                        }
        ...
        var resetButton = $(
                '<button class="btn btn-warning btn-sm dt-reset-filter-button" data-title="Wyczyszczenie kryteriów wyszukiwania" data-toggle="tooltip"><i class="fa fa-eraser"></i></button>'
                );
        resetButton
        ...
        if (operationsFooter.find(".dt-select-all-button").size()) {
            operationsFooter.find(".dt-select-all-button").on("click", function (e) {
                e.stopPropagation();
                e.preventDefault();
                var checkboxes = table.find("tbody").find(".icheckbox_square-aero");
        ...
        var changeLimitButton = $(
                '<div class="dt-change-limit-section"><div class="input-group"><select class="form-control"></select><span class="input-group-btn show-notify processed" data-notify-text="#notify_trigger_mode_4_day"><button type="button" class="btn btn-default" data-toggle="tooltip" data-placement="left" data-title="' +
                uiTranslate("Ilość wyników na stronie") +
                '"><i class="fa fa-th-list"></i></button></span></div></div>'
                );
        changeLimitButton
        ...
        paginationSection.find(".buttons-colvis").tooltip({
            title: uiTranslate("Wybierz kolumny"),
            container: "body"
        });
    ...
    var getSearchParams = function (table, mode) {
        ...
        return mode === "url"
                ? helperForm.serialize()
                : helperForm.serializeObject();
    };
    ...
    var updateChangeLimit = function () {
        ...
        select.html(original.html()).val(original.val());
    };
    var uniqueId = 0;
    ...
    $("table.example")
            ...
            .each(function () {
                ...
                table.addClass(tableMaxId).attr("data-table-max-id", tableMaxId);
                ...
                table.find("thead th").each(function () {
                    ...
                    if (filterType) {
                        ...
                        switch (filterType) {
                            ...
                            case "date-range":
                                filter = $(
                                        '<div class="filter-data-range input-group input-daterange">'
                                        );
                                var filterFromId = "filer-from-id-" + uniqueId,
                                        filterToId = "filer-to-id-" + uniqueId;

                                var filterFrom = $(
                                        '<input type="text" class="filter-data-range-from datepicker-input datepicker-relative" placeholder="' +
                                        uiTranslate("od") +
                                        '">'
                                        )
                                        .attr({
                                ...
                                var filterTo = $(
                                        '<input type="text" class="filter-data-range-to datepicker-input datepicker-relative" placeholder="' +
                                        uiTranslate("do") +
                                        '">'
                                        )
                                        .attr({
                                ...
                                filter.append(filterFrom).append(filterTo);
...
toggleRelativeInit = function () {
    $(".toggle-relative")
            ...
            .each(function () {
                $(this)
                        .on("change", function () {
                            var relativeElements = $(".form-group, .relative-element").filter(
                                    '[data-relation-base="' + this.id + '"]'
                                    );
                            relativeElements
                                    ...
                                    .filter(
                                            '[data-relation-id="' +
                                            this.value +
                                            '"], [data-relation-id="all"], [data-relation-ids^="' +
                                            this.value +
                                            '"], [data-relation-ids$=",' +
                                            this.value +
                                            '"], [data-relation-ids*=",' +
    ...
    $(".clear-relative")
            ...
            .each(function () {
                $(this).on("change", function () {
                    var relativeElements = $(".form-group, .clear-relative-element").filter(
                            '[data-clear-relative-base="' + this.id + '"]'
                            );
...
nestableInit = function () {
    $(".nestable")
            ...
            .each(function () {
                ...
                tg.find(".dd-handle").on("click", expandItem);
            })
...
confirmDeleteInit = function () {
    $(".confirm-delete")
            ...
            .each(function (e) {
                ...
                tg.attr("id", "confirm-delete-" + uniqueId.get()).confirmation({
...
modalConfirmDelete = function () {
    var baseText =
            "<h3>Potwierdź operację</h3><div>Czy na pewno chcesz usunąć wybrane elementy?</div>";
    ...
    $(".modal-confirm-delete")
            ...
            .each(function (e) {
                ...
                tg.on("click", function (e) {
                    ...
                    var checkboxes = table.find("tbody").find(".icheckbox_square-aero");
                    var modalText = $(textSelector).size()
                            ? $(textSelector).html()
                            : baseText;
                    ...
                    if (checkboxes.filter(".checked").size() === 0) {
                        modalText =
                                "<h3>Potwierdź operację</h3><div>aby użyć funkcji usun, musisz zaznaczyć przynajmniej jeden wpis</div>";
                    }

                    bootbox.confirm(modalText, function (result) {
                        if (result == true && checkboxes.filter(".checked").size() != 0) {
...
modalConfirmInit = function () {
    var baseTexts = {
        singleDelete:
                "<h3>Potwierdź operację</h3><div>Czy na pewno chcesz usunąć wybrany element?</div>"
    };
    ...
    $(".modal-confirm")
            ...
            .each(function (e) {
                var tg = $(this),
                        ...
                        confirmText = this.hasAttribute("data-confirmation-text")
                        ? this.getAttribute("data-confirmation-text")
                        : baseTexts[confirmClass],
                        processMoreFn = this.hasAttribute("data-process-more")
                        ? $.proxy(window[this.getAttribute("data-process-more")], tg)
                        : $.noop;
                        ...
                        callback: function (result) {
                            if (result == true) {
                                if (isAjax) {
                                    ...
                                    tg.trigger("click.api", {
                                        "modal-confirm": {confirmed: true}
                                    });
                                } else {
            ...
            .each(function (e) {
                ...
                tg.on("click", function () {
                    ...
                    $(".modal.in")
                            .stop()
                            .animate({
                                scrollTop: ($(".modal.in").get(0).scrollTop += target.height())
                            });
                });
            })
    ...
    $(".widget-copy-value-as-text")
            ...
            .each(function (e) {
                var tg = $(this),
                        ...
                        filterFn = tg.attr("data-copy-filter-fn")
                        ? window[tg.attr("data-copy-filter-fn")]
                        : function (a) {
                            return a;
                        },
...
icheckDumbInit = function () {
    ...
    var elem = $(
            "input:not(.ios-switch, .angular, .noprocess, .processed-icheckDumb)"
            );
...
messagesTableInit = function () {
    ...
    function updateTagCounter(tag) {
        var tagListItem = $(".messages-tags").find(
                ".list-group-item[data-id=" + tag.id + "]"
                ),
    ...
    function updateFolderCounter(folder) {
        var folderListItem = $(".menu-message").find(
                ".list-group-item[data-id=" + folder.id + "]"
                ),
    ...
    function messageDisplayTag(messageId, tagId, mode) {
        var messageRow = messageList.find(
                ".message-row[data-id=" + messageId + "]"
                );
        if (tagId == 1) {
            if (mode === "add") {
                messageRow
            ...
            if (mode === "add") {
                ...
                if (label.size() === 0) {
                    var tagLabel = $("<span>")
                            ...
                            .css(
                                    "background-color",
                                    tagListItem.find(".fa-circle").css("color")
                                    )
    ...
    function getSelectedIds() {
        ...
        messageList
                .find(".message-row input[type=hidden]")
                .filter("[value=1]")
                .each(function () {
    ...
    ajaxHeartbeat = function () {
        $.ajax({
            ...
            success: function (response) {
                ...
                if (
                        response.lastMessageDate !== storage.lastMessageDate &&
                        response.lastMessageDate > 0
                        ) {
                    exp.show(
                            notifications.lastMessageDate[0],
                            notifications.lastMessageDate[1],
                            notifications.lastMessageDate[2]
                            );
                }

                if (response.hasUnreadKomunikat && !storage.hasUnreadKomunikat) {
                    exp.show(
                            notifications.unreadKomunikat[0],
                            notifications.unreadKomunikat[1]
                            );
                ...
                if (response.termsAccepted == "0" && homeWelcome !== "home/welcome") {
...
initializeDatatables = function () {
    var languagePack = {
        pl: {
            ...
            zeroRecords:
                    "Nie znaleziono rekordów spełniających podane kryteria wyszukiwania",
            emptyTable: "Brak rekordów",
            infoEmpty: "Brak rekordów",
            info:
                    'Rekordy: <b><span class="paginate_start_text">_START_</span></b> - <b><span class="paginate_end_text">_END_</span></b> z <span class="paginate_total_text">_TOTAL_</span>',
        ...
        en: {
            ...
            info:
                    'Display <b><span class="paginate_start_text">_START_</span></b><span class="three-dot">…</span><b><span class="paginate_end_text">_END_</span></b> from <span class="paginate_total_text">_TOTAL_</span>',
    ...
    $(".example, #example")
            ...
            .each(function () {
                var tg = $(this),
                        sortByColumn = 0,
                        ths = tg.find("thead tr:last th"),
                        thSize = ths.size(),
                        sortingOption = [],
                        dataFirstColumn = tg.find("tbody td:first-child"),
                        dataSortColumn,
                        colvisInclude = [],
                        ajaxSourceUrl = tg.attr("data-source-url")
                        ? tg.attr("data-source-url")
                        : null;
                ...
                if (thSize) {
                    ...
                    ths.each(function () {
                        ...
                        if ($(this).attr("data-column-name")) {
                            sortingOption[operationsTdIndex]["data"] = $(this).attr(
                                    "data-column-name"
                                    );
                        }
                        if ($(this).attr("data-column-class")) {
                            sortingOption[operationsTdIndex]["className"] = $(this).attr(
                                    "data-column-class"
                                    );
                        }
                    });
                    ...
                    if (
                            dataFirstColumn.find("input[type=checkbox], .iCheck-helper").size()
                            ) {
                        sortingOption[0]["orderDataType"] = "dom-checkbox";
                        sortByColumn = 1;
                    ...
                    if (sortByColumn === 0) {
                        ...
                        dataSortColumn = tg.find(
                                "tbody td:nth-child(" + (sortByColumn + 1) + ")"
                                );
                    }
                ...
                var dtObject = tg.dataTable({
                    fnDrawCallback: function () {
                        tg.find("input:not(.ios-switch, .angular)").each(function () {
                            if (
                                    ...
                                    !$(this)
                                    .parent()
                                    .hasClass("icheckbox_square-aero")
                                    ) {
                                $(this).iCheck({
                                    checkboxClass: "icheckbox_square-aero",
                                    radioClass: "iradio_square-aero",
                    ...
                    sDom:
                            "<'dt-hidden hiddenElement'l><'row dt-header'<'col-md-5 dt-header-operations'i><'col-md-7 dt-pagination'pB>>rt<'row dt-footer'<'col-md-6 dt-footer-operations'><'col-md-6 dt-pagination'p>>",
                ...
                /*-----following search only works for registry dropdown in doc template page----*/
                if (
                        $("#registry_id").length > 0 &&
                        $("#registry_id")[0].nodeName.toLowerCase() === "select"
                        ) {
                    if (
                            $("#registry_id option:selected").text() != undefined &&
                            $("#registry_id option:selected").text() != ""
                            ) {
                        dtObject.fnFilter($("#registry_id option:selected").text());
                    }
                    $("#registry_id").change(function () {
                        var registryText = $("#registry_id option:selected").text();

                        if (registryText != "" && registryText != undefined) {
                            dtObject.fnFilter(registryText);
                        }
                    });
                }
                /*-----above search only works for registry dropdown in doc template page----*/
...
var widget = (function () {
    ...
    var initializer = function (
        ...
        $.each(_definitions, function () {
            ...
            if (
                    typeof blockInit[this.class] !== "undefined" &&
                    !testBlockInit(this, context)
                    ) {
                repeat = true;
                go = false;
            }
    ...
    var testBlockInit = function (definition, context) {
        var go = true,
                testContext = context
                .find(definition.classSelector)
                .not(definition.classProcessed);
        ...
        if (typeof blockInit[definition.class] !== "undefined") {
            $.each(blockInit[definition.class], function (__idx, blockerClass) {
                var blockerDefinition = definitions[blockerClass],
                        ...
                        notProcessedContextElements = contextElements.not(
                                blockerDefinition.classSelectorProcessed
                                );
...
ckeditorLiveSaveIntegration = function (editor, tg) {
    if (tg.closest(".live-save").size()) {
        timeouter.registerStorage("live-save-ckeditor", true, {
            unloadBefore: ["live-save"]
        });
...
ckeditorAutohide = function (editor) {
    ...
    editor.on("loaded", function (e) {
        ...
        tg.find(".cke_top, .cke_bottom").on("click", function (e) {
            if (e.target == this) {
                editor.focus();
            }
        });
...
googlemapAssets = {
    sync: [
        "https://maps.googleapis.com/maps/api/js?key=AIzaSyAuLAivouPtepNEeh06uEoBQr-V8EnaMbg&libraries=places"
    ]
};

widget.push({
    class: "googlemap",
    scripts: googlemapAssets,
    blockInit: ["live-save"],
    initElement: function (tg) {
        var defaultLocation = {
            lat: 31.418,
        ...
        if ($(".longlatval").val() != "") {
            var dataLocation = $(".longlatval").val();
            dataLocation = dataLocation.split("|");
            defaultLocation.lat = parseFloat(dataLocation[1]);
            defaultLocation.long = parseFloat(dataLocation[0]);
        }
        var marker;
        ...
        //event listener google map marker dragged
        google.maps.event.addListener(marker, "dragend", function (event) {
            map.setCenter(event.latLng);
            $(".longlatval").val(
                    event.latLng.lng().toFixed(4) + "|" + event.latLng.lat().toFixed(4)
                    );
        });

        //event listener google map search box address selected
...
widget.push({
    ...
    initElement: function (tg) {
        var closestElement = tg.closest(
                tg.attr("data-closest-target")
                ? tg.attr("data-closest-target")
                : document
                ),
                targetElement = tg.attr("data-target"),
                target = targetElement
                ? closestElement.find(targetElement)
                : closestElement;
...
widget.push({
    ...
    initElement: function (tg) {
        var processMoreFn = tg.attr("data-process-more")
                ? $.proxy(window[tg.attr("data-process-more")], tg)
                : $.noop;
...
bootstrapCalendarScripts = {
    sync: [
        uiLanguage === "pl"
                ? "/assets/plugins/bootstrap-calendar-0.2.5/js/language/pl-PL.js"
                : "/assets/plugins/bootstrap-calendar-0.2.5/js/language/pl-PL.js",
    ...
    initElement: function (tg) {
        ...
        var options = {
            ...
            onAfterViewLoad: function (view) {
                ...
                buttons
                        .filter('[data-calendar-view="' + view + '"]')
                        .addClass("active");
...
widget.push({
    ...
    initElement: function (tg) {
        ...
        var addRow = function (objectData) {
            ...
            if (
                    lastSibling.size() &&
                    lastSibling.find(sourceInputSelector).val().length
                    ) {
...
function replaceInAttribute(el, attributeName, oldValue, newValue) {
    if (el.hasAttribute(attributeName)) {
        el.setAttribute(
                attributeName,
                el.getAttribute(attributeName).replace(oldValue, newValue)
                );
    }
}
...
$(function () {
    systemAssignHandlers();
    $(".email-valid-check").blur(function () {
              // validate email
        if ($(".email-valid-check").size()) {
            email = $('#email').val();
            if (!validateEmail(email)) {
                ...
                $('#emailError').hide();
                var dataString = 'email=' + jQuery('#email').val();
                var hiddenId = parseInt(jQuery('#id').val());
                $.ajax({
                    type: 'post',
                    url: "/ajax/check-useremail",
                    data: dataString,
                }).done(function (data) {
                    if ((data.status == 0)|| (parseInt(data.status) == hiddenId)) {
                        jQuery('#unique-email-error').hide();
                         jQuery('.unique-email-error').hide();
                        $("#submit_type").removeAttr("disabled");
                    } else {
                        jQuery('#unique-email-error').show();
                         jQuery('.unique-email-error').show();
                        $("#submit_type").attr("disabled", true);
                    } //end of if-else
                });
            }
