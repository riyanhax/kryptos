...
Sidebar.prototype.init = function()
{
	...
	STENCIL_PATH = '/assets/flowchartassets/stencils';
	var dir = '/assets/flowchartassets/stencils';
	var registryJson = [];
	var saveThis = this;

	$.post("/Registry/diagramd", { id: 11 },
                    //callback function
	    function (respond) {
	        var diagramString = respond;
	         registryJson = JSON.parse(respond);
	           
	         	saveThis.addSearchPalette(true);
	         	for(var i = 0 ; i < registryJson.length ;i++){
	         	var str = registryJson[i];
	         	var regVal = str.split(",");
	         	saveThis.addGenerallPalette(false,regVal[0],regVal[1]);

	         }

				saveThis.addGeneralPalette(false);
				
				saveThis.addMiscPalette(false);
				saveThis.addAdvancedPalette(false);
				saveThis.addBasicPalette(dir);
				saveThis.addStencilPalette('arrows', mxResources.get('arrows'), dir + '/arrows.xml',
					';whiteSpace=wrap;html=1;fillColor=#ffffff;strokeColor=#000000;strokeWidth=2');
				saveThis.addUmlPalette(false);
				saveThis.addBpmnPalette(dir, true);
				saveThis.addStencilPalette('flowchart', 'Flowchart', dir + '/flowchart.xml',
					';whiteSpace=wrap;html=1;fillColor=#ffffff;strokeColor=#000000;strokeWidth=2');
				saveThis.addImagePalette('clipart', mxResources.get('clipart'), dir + '/clipart/', '_128x128.png',
					['Earth_globe', 'Empty_Folder', 'Full_Folder', 'Gear', 'Lock', 'Software', 'Virus', 'Email',
					 ...
					 'Worker1', 'Soldier1', 'Doctor1', 'Tech1', 'Security1', 'Telesales1', 'hot_coffee'], null,
 ...
 */
Sidebar.prototype.gearImage = '/assets/flowchartassets/stencils/clipart/Gear_128x128.png';
...
///////////
Sidebar.prototype.addGenerallPalette = function(expand,id,name)
{
	var lineTags = 'line lines connector connectors connection connections arrow arrows ';
	var fns = [];
	var saveThis = this;

	$.post('/registry-entries/getentities', { rid: id},
        function (respond) {
  
			result=JSON.parse(respond);

			for (var i = 0; i < result.length; i++) {
				fns[i] =  saveThis.createVertexTemplateEntry('shape=ext;rounded=1;html=1;whiteSpace=wrap;strokeWidth=3;', 120, 80, result[i], result[i], null, null, 'bpmn call activity');
			    
			}
			saveThis.addPaletteFunctions(id, name , (expand != null) ? expand : true, fns);
	...
	// this.addPaletteFunctions(name, mxResources.get('general'), (expand != null) ? expand : true, fns);
};
/**
 * Adds the general palette to the sidebar.
 */
